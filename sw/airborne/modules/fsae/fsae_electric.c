...
void fsae_electric_periodic(void)
{
  ...
  // read inputs
  if (flag) {
    stg_in = gpio_read(GPIOE, 7); //  PE7  - Digital input. (DIN1:DIN_STG1).
    flag = false;
  }
  else {
    stb_in = gpio_read(GPIOE, 9); // PE9 - Digital input. (DIN3:DIN_STB1).
    flag = true;
  }
*/
  ...
  //dac_ref+=100;
  //dacPutChannelX(&DACD1,0,dac_ref);
  //dacPutChannelX(&DACD2,1,dac_ref);
  //dac_ref+=100;
}


void fsae_electric_UpdateDac1(uint16_t val) {
  dac_1 = val;
  dac_ref1 = dac_1;
  dacPutChannelX(&DACD1,0,dac_ref1);
}

void fsae_electric_UpdateDac2(uint16_t val) {
  dac_2 = val;
  dac_ref2 = dac_2;
  dacPutChannelX(&DACD2,1,dac_ref2);
}
